-- Sample Data for Ecommerce Application (PostgreSQL Version)
...
-- Insert Orders (PostgreSQL INTERVAL syntax)
...
('order-001', 'ORD-2024-001', 'user-001', 1049.97, 104.99, 15.00, 1169.96, 'CONFIRMED', 'PAID', 'CREDIT_CARD', '123 Main Street', 'New York', 'NY', '10001', 'USA', '+1-555-0101', CURRENT_TIMESTAMP + INTERVAL '5 days', 'TRK123456789', 'Please deliver during business hours', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('order-002', 'ORD-2024-002', 'user-002', 1999.99, 199.99, 25.00, 2224.98, 'SHIPPED', 'PAID', 'CREDIT_CARD', '456 Oak Avenue', 'Los Angeles', 'CA', '90210', 'USA', '+1-555-0102', CURRENT_TIMESTAMP + INTERVAL '3 days', 'TRK987654321', 'Signature required', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('order-003', 'ORD-2024-003', 'user-003', 59.99, 6.00, 10.00, 75.99, 'DELIVERED', 'PAID', 'DEBIT_CARD', '789 Pine Street', 'Chicago', 'IL', '60601', 'USA', '+1-555-0103', CURRENT_TIMESTAMP - INTERVAL '2 days', 'TRK456789123', 'Left with neighbor', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
('payment-001', 'order-001', 'user-001', 1169.96, 'CREDIT_CARD', 'SUCCESSFUL', 'TXN123456789', 'Approved', '1234', 'VISA', 12, 2026, '123 Main Street', 'New York', 'NY', '10001', 'USA', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('payment-002', 'order-002', 'user-002', 2224.98, 'CREDIT_CARD', 'SUCCESSFUL', 'TXN987654321', 'Approved', '5678', 'MASTERCARD', 8, 2025, '456 Oak Avenue', 'Los Angeles', 'CA', '90210', 'USA', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('payment-003', 'order-003', 'user-003', 75.99, 'DEBIT_CARD', 'SUCCESSFUL', 'TXN456789123', 'Approved', '9012', 'VISA', 3, 2027, '789 Pine Street', 'Chicago', 'IL', '60601', 'USA', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
-- Insert more sample orders for demonstration (PostgreSQL INTERVAL syntax)
INSERT INTO orders (id, order_number, user_id, subtotal, tax_amount, shipping_amount, total_amount, status, payment_status, payment_method, delivery_address, delivery_city, delivery_state, delivery_zip_code, delivery_country, delivery_phone, estimated_delivery_date, tracking_number, notes, created_at, updated_at) VALUES
('order-004', 'ORD-2024-004', 'user-004', 89.99, 9.00, 10.00, 108.99, 'PROCESSING', 'AUTHORIZED', 'CREDIT_CARD', '321 Elm Street', 'Boston', 'MA', '02101', 'USA', '+1-555-0104', CURRENT_TIMESTAMP + INTERVAL '7 days', 'TRK789123456', 'Apartment building - call upon arrival', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('order-005', 'ORD-2024-005', 'user-001', 149.99, 15.00, 15.00, 179.99, 'PENDING', 'PENDING', 'DIGITAL_WALLET', '123 Main Street', 'New York', 'NY', '10001', 'USA', '+1-555-0101', CURRENT_TIMESTAMP + INTERVAL '10 days', NULL, 'Second order from this customer', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP);
...
-- Reset sequences if using SERIAL or BIGSERIAL (PostgreSQL specific)
-- Uncomment the following lines if you're using SERIAL/BIGSERIAL instead of UUID
-- SELECT setval('users_id_seq', (SELECT MAX(CAST(REPLACE(id, 'user-', '') AS INTEGER)) FROM users), true);
-- SELECT setval('categories_id_seq', (SELECT MAX(CAST(REPLACE(id, 'cat-', '') AS INTEGER)) FROM categories), true);
-- SELECT setval('products_id_seq', (SELECT MAX(CAST(REPLACE(id, 'prod-', '') AS INTEGER)) FROM products), true);
-- SELECT setval('carts_id_seq', (SELECT MAX(CAST(REPLACE(id, 'cart-', '') AS INTEGER)) FROM carts), true);
-- SELECT setval('cart_items_id_seq', (SELECT MAX(CAST(REPLACE(id, 'cartitem-', '') AS INTEGER)) FROM cart_items), true);
-- SELECT setval('orders_id_seq', (SELECT MAX(CAST(REPLACE(id, 'order-', '') AS INTEGER)) FROM orders), true);
-- SELECT setval('order_items_id_seq', (SELECT MAX(CAST(REPLACE(id, 'orderitem-', '') AS INTEGER)) FROM order_items), true);
-- SELECT setval('payments_id_seq', (SELECT MAX(CAST(REPLACE(id, 'payment-', '') AS INTEGER)) FROM payments), true);
